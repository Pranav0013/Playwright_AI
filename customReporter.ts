...
class CustomTTAReporter implements Reporter {
  ...
  onBegin(config: FullConfig, suite: Suite): void {
    ...
    // );
    // console.log(
    //   `║  📅 Started: ${this.startTime.toLocaleString().padEnd(47)}║`,
    // );
  ...
  onTestEnd(test: TestCase, result: TestResult): void {
    ...
    for (const step of currentTestSteps) {
      for (const [name, screenshotPath] of stepScreenshots) {
        ...
        if (
          nameLower.includes(stepNumPattern1) ||
          nameLower.includes(stepNumPattern2)
        ) {
          step.screenshot = screenshotPath;
          break;
        }
        ...
        if (
          cleanedName &&
          (titleLower.includes(cleanedName) ||
            cleanedName.includes(titleLower.substring(0, 20)))
        ) {
          step.screenshot = screenshotPath;
          break;
        }
  ...
  async onEnd(_result: FullResult): Promise<void> {
    ...
    console.log(
      `║  ⏭️  Skipped: ${String(this.suiteStats.skipped).padEnd(49)}║`,
    );
  ...
  private associateLogsWithSteps(
    ...
    if (unassignedLogs.length > 0 && testSteps.length > 0) {
      ...
      const stepsNeedingLogs = testSteps.filter(
        (s) => s.consoleLogs!.length === 0,
      );
      ...
      if (stepsNeedingLogs.length > 0) {
        ...
        for (
          ...
          const logsForThisStep = Math.min(
            logsPerStep,
            unassignedLogs.length - logIdx,
          );
...
<body>
    ...
    <div class="report-list">
        ${files
          .map((f, i) => {
            const match = f.match(
              /report_(\d{4})(\d{2})(\d{2})_(\d{2})(\d{2})(\d{2})\.html/,
            );
            const dateStr = match
...
<body>
  ...
  private generateTestTable(): string {
    ...
    for (const test of this.testResults) {
      ...
      const testGroup =
        test.tags.find(
          (t) => t.includes("P0") || t.includes("P1") || t.includes("P2"),
        ) ||
        test.describePath[0] ||
        "E2E";
